...
var simpleFSM = function () {
  ...
  // The terminal states, where part of pattern needs to be marked.
  var markStates = Object.create( null );
  // Add-ons value is stored here.
  var addonsAtStates = Object.create( null );
  // Reverse of the above!
  var name2state = Object.create( null );
  //
  var propAtState = Object.create( null );
  ...
  */
   ...
   * @param {any} addons contains definable value(s).
   * @returns {undefined} Nothing!
   * @private
  */
  var learnSinglePattern = function ( name, pattern, mark, addons ) {
    ...
    const target = undefined; // name2state[ name ];
    ...
    for ( let k = 0; k < length; k += 1 ) {
      const pk = pattern[ k ].split( ':' );
      let property;
      if ( pk.length === 1 ) {
        ev = pk[ 0 ];
        property = '';
      } else {
        property = pk.shift().trim();
        ev = pk.join( '' ).trim();
      }

      // ev = pattern[ k ];
      propAtState[ state ] = propAtState[ state ] || [];
      propAtState[ state ].push( property );
    ...
    if ( mark ) {
      mark[ 1 ] = length - mark[ 1 ] - 1;
      markStates[ state ] = mark;
    }

    if ( addons !== undefined ) {
      addonsAtStates[ state ] = addons;
    }
  }; // learnSinglePattern()
  ...
  */
    ...
    for ( let i = 0; i < patterns.length; i += 1 ) {
      ...
      if ( typeof pi.pattern === 'string' ) {
        ...
        for ( let j = 0; j < all.length; j += 1 )
          learnSinglePattern( pi.name, all[ j ], pi.mark, pi.addons );
      } else learnSinglePattern( pi.name, pi.pattern, pi.mark, pi.addons );
    }
   ...
   * the detected pattern had an `addons` property defined.
   * @param {function} f to be called with `match` & `addons` value as parameters.
  ...
  */
   ...
   * marking and calling the on pattern detection function, if required.
   *
   * @param {array} patterns where the `match` is pushed.
   * @param {array} match pushed in to the `patterns`.
   * @returns {undefined} Nothing.
   * @private
  */
  var pushMatch2Patterns = function ( patterns, match ) {
    ...
    var mark = markStates[ name2state[ match[ 0 ] ] ];
    var addons = addonsAtStates[ name2state[ match[ 0 ] ] ];
    if ( mark ) {
      match[ 1 ] += mark[ 0 ];
      match[ 2 ] -= mark[ 1 ];
    }

    if ( addons !== undefined && onPatternDetectionFn )
      onPatternDetectionFn( match, addons );
  ...
  /**
   *
   * Recognizes **all** patterns present in the input tokens; patterns could be
   * overlapping or contained in other patterns as well. The patterns are
   * learned via the `learn()` api.
   *
  ...
  */
  var recognize = function ( tokens ) {
    ...
    var patterns = [];
    // Create the place holder for machines.
    var machines = Object.create( null );
    // Create the first machine.
    machines[ 0 ] = Object.create( null );
    // Points to the token at which this machine was fired.
    machines[ 0 ].first = -1;
    machines[ 0 ].state = root;
    machines[ 0 ].id = 0;
    // Next machine to be created.
    let next = 1;
    // A FSM itself can have 3 states: `ready`, `chasing` and `idling`.
    //
    // A `ready` FSM is the one which is ready to start chasing patterns.
    // It starts chasing likely pattern(s) on arrival of first candidate event.
    // At any time, there can only be one FSM that is ready.
    // Once the `ready` FSM starts `chasing`, a new FSM is spinned in to
    // the `ready` state.
    //
    // FSM stops chasing pattern(s) whenever it reaches (a) the terminal state on
    // successful detection of a pattern or (b) the root state on failing to
    // detect a pattern. At this stage, such a machine is moved to `idling` state.
    //
    // While readying a machine, we attempt to  use of an `idling` machine
    // instead of creating a new one.
    var ready = machines[ 0 ];
    var chasing = Object.create( null );
    var idling = [];
    // Temps:
    // Machine, popped from idling list, token
    var m, popped, t;
    // Loop index and Machine id.
    var i, mid;
    // A pattern.
    var apat;

    // Iterate through each token and send it to (a) the `ready` machine and
    // (b) each machine that is `chasing`. Remember to iterate upto `<= length`
    // instead of `< length` to push end-of-sentence token.
    for ( i = 0; i <= tokens.length; i += 1 ) {
      t = ( i === tokens.length ) ?  eosToken : tokens[ i ];
      // Change machine's state if the current token fires the `ready` machine
      // as discussed above. NOTE: there is always a ready machine at this point;
      // refer to the next if-block.
      if ( fsm[ ready.state ][ t ] ) {
        ready.first = i;
        chasing[ ready.id ] = true;
        // This machine is no longer `ready`.
        ready = null;
      }

      if ( ready === null ) {
        // No machine is ready, spin a new machine — if there are no `idling`
        // machine(s)  create a new machine otherwise try to use an existing
        // `idling` machine.
        if ( idling.length === 0 ) {
          machines[ next ] = Object.create( null );
          machines[ next ].first = -1;
          machines[ next ].state = root;
          machines[ next ].id = next;
          // Ensure a `ready` machine.
          ready = machines[ next ];
          next += 1;
        } else {
          popped = idling.pop();
      ...
      // Iterate through each machine that is currently chasing. Note ideally this
      // should have been a list but in current context maintaining it as a list
      // is an overkill; so it is being maintained as a JS object. As a result,
      // we need another if-block to identify machines that are truly chasing!
      for ( mid in chasing ) {
        m = machines[ mid ];
        if ( chasing[ mid ] ) {
          // Check for terminal states and accordingly add to the detected `patterns`.
          if ( terminalStates[ fsm[ m.state ][ t ] ] ) {
            apat = [ terminalStates[ fsm[ m.state ][ t ] ], m.first, i ];
            pushMatch2Patterns( patterns, apat );
          }

          // Note terminal state can be found with `otherwise` also in case of a
          // single pattern overlapping with others from the left edge.
          if ( terminalStates[ fsm[ m.state ][ otherwise ] ] ) {
            apat = [ terminalStates[ fsm[ m.state ][ otherwise ] ], m.first, i - 1 ];
            pushMatch2Patterns( patterns, apat );
          }

          // Assign the next state.
          m.state = fsm[ m.state ][ t ] || fsm[ m.state ][ otherwise ];

          // Machine can't run if the state has moved to `root` or a terminal
          // state has been reached: add the machine to `idling` list.
          if ( m.state === 0 || terminalStates[ m.state ] ) {
            m.first = -1;
            // It is no longer truly chasing.
            chasing[ mid ] = false;
            idling.push( m );
          } // if terminal state is reached
        } // if machine is still firing
      } // for each machine in chasing list
    } // for each token

    // Sort patterns in the right sequence i.e. from left to right.
    ...
    // All done — return whatever `patterns` have been recognized.
    return patterns;
  }; // recognize()
   ...
   * `greedilyRecognize()` api.
   *
   * @param {array[]} patterns to be used for substitutions in `greedilyRecognize()`.
   * @returns {undefined} Nothing.
   * @private
  */
  var setPatternSwap = function ( patterns ) {
    ...
    patterns.sort( ( a, b ) => ( a[ 1 ] > b[ 1 ] ) );
    // Index it by start of pattern.
    patterns.forEach( ( e ) => ( substitutions[ e[ 1 ] ] = [ e[ 2 ], e[ 0 ] ] ) );
  }; // setPatternSwap()
   ...
   * `[ pattern-id, start-token, end-token ]` format.
  */
  var greedilyRecognize = function ( tokens, transformToken, param ) {
    ...
    for ( let i = 0; i <= length; i += 1 ) {
      ...
      for ( let j = i; j <= length; j += delta ) {
        ...
        // for the `eosToken`.
        if ( transformTokenFn && ( j < length ) ) t = transformTokenFn( t, j, param );
        ...
        if ( terminalStates[ ns ] ) {
          ...
          p = [ terminalStates[ ns ], first, j + delta - 1 ];
          ...
          if ( lastOtherwiseState ) {
            ...
            p = [ terminalStates[ lastOtherwiseState ], first, lastOtherwiseIndex ];
            pushMatch2Patterns( patterns, p );
  ...
  var printModel = function () {
    ...
    console.log();
    console.log( 'Mark States:' );
    console.log( JSON.stringify( markStates, null, 2 ) );
  }; // printModel()


  methods.learn = learn;
  methods.recognize = recognize;
  methods.greedilyRecognize = greedilyRecognize;
...
var f = simpleFSM();
var patterns = [
  { name: 'club', pattern: [ 'pos:Mr', 'Sanjaya', 'Kumar', 'Saxena' ], mark: [ 1, 3 ], addons: 99 },
  { name: 'city', pattern: [ 'm' ] },
];
f.learn( patterns );
f.setOnPatternDetectionFn( ( match, addons ) => ( match.push( addons ) ) );
console.log( f.greedilyRecognize( [ 'I', 'met', 'Mr', 'Sanjaya', 'Kumar', 'Saxena', 'yesterday', '.' ] ) );
f.printModel();
