...
const eosToken = 2070000;

var simpleFSM = function ( cache ) {
  // Returned!
  ...
  // Extract keys to ignore during entity detection. If no cache was null then
  // set key to `null` — critical for model generation.
  var keyLF = ( cache ) ? cache.lookup( '\n' )[ 0 ] : null;
  // var keyCRLF = cache.lookup( '\r\n' )[ 0 ];
  // var keyCR = cache.lookup( '\r' )[ 0 ];
    ...
    for ( let i = 0; i < patterns.length; i += 1 ) {
      ...
      if ( typeof pi.pattern === 'string' ) {
        ...
        for ( let j = 0; j < all.length; j += 1 )
          learnSinglePattern( pi.name, all[ j ], pi.mark, pi.customProperty );
      } else learnSinglePattern( pi.name, pi.pattern, pi.mark, pi.customProperty );
    }
      ...
      for ( mid in chasing ) {
        ...
        if ( chasing[ mid ] ) {
          ...
          if ( terminalStates[ fsm[ m.state ][ otherwise ] ] ) {
            apat = [ m.first, i - 1, fsm[ m.state ][ otherwise ] ];
            pushMatch2Patterns( patterns, apat );
          }
...
// var f = simpleFSM();
// var patterns = [
//   { name: 'club', pattern: [ 'Mr', 'Sanjaya', 'Kumar', 'Saxena' ], mark: [ 1, 3 ], customProperty: 99 },
//   { name: 'city', pattern: [ 'm' ] },
// ];
// f.learn( patterns );
// f.setOnPatternDetectionFn( ( match, customProperty ) => ( match.push( customProperty ) ) );
// console.log( f.greedilyRecognize( [ 'I', 'met', 'Mr', 'Sanjaya', 'Kumar', 'Saxena', 'yesterday', '.' ] ) );
// f.printModel();
// console.log( f.exportJSON() );
